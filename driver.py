...
def stockprediction():
	...
	msg= (df[criteria].loc[:,['nday_cng']].agg(['mean','std']))
	p=str(msg)
	m=p.split()
	msg1=m[0]
	msg2=m[1]
	...
	msg4=m[3]
	msg5=m[4]
	res=float (msg3)+ float (msg5)
	return render_template('stockresult.html',msg1 = msg1, msg2=msg2, msg3=msg3, msg4=msg4, msg5=msg5, res=res)
...
@app.route('/loggednetwork') #homepage 
def loggednet():
	return render_template('afternetlogin.html')
...
def charts():
    ...
    msg3=(list[2]/sum)*360
    msg4=(list[3]/sum)*360
    return render_template('chart.html', msg1=msg1, msg2=msg2, msg3=msg3, msg4=msg4, msg5=list[0], msg6=list[1], msg7=list[2], msg8=list[3])
@app.route("/count")
def count():
    list=[]
    list.append(countusers('AIRTEL'))
    ...
    msg3=list[2]
    msg4=list[3]
    return render_template('result3.html', msg1=msg1, msg2=msg2, msg3=msg3, msg4=msg4)
	
#OFFERS
@app.route('/addoffers/<msg1>')  
def addoffers(msg1):
	return render_template('addoffers.html',msg1=msg1)

@app.route('/storeoffers',methods=['POST','GET']) 
def storeoffers():
		if request.method=='POST':
		#try:
			name=request.form['name']
			...
			with sql.connect('minidatabase.db')as con:
				cur=con.cursor()
				cur.execute('INSERT INTO offer(name,price,value,validity) VALUES(?,?,?,?)',(name,price,value,validity))
...
@app.route('/offer/<net>') #homepage 
def offer(net):
	con=sql.connect('minidatabase.db')
	con.row_factory = sql.Row
	
	cur=con.cursor()
	cur.execute('select * from offer where name=?',(net,))
	# cur.execute('select * from plan')
	rows=cur.fetchall();
	return render_template('offertable.html',rows=rows)
	
@app.route('/alloffers') #homepage 
def alloffers():
	con=sql.connect('minidatabase.db')
	con.row_factory = sql.Row
...
@app.route('/usereviewparticular/<name>') #homepage 
def userreviewparticular(name):
	con=sql.connect('minidatabase.db')
	con.row_factory = sql.Row
	
	cur=con.cursor()
	cur.execute('select * from reviews where net=?',(name,))
	# cur.execute('select * from plan')
	rows=cur.fetchall();
	return render_template('review2table.html',rows=rows)

	
@app.route('/filter/<net>') #homepage 
def filter(net):
	con=sql.connect('minidatabase.db')
	con.row_factory = sql.Row
	
	cur=con.cursor()
	cur.execute('select * from reviews where net=?',(net,))
	# cur.execute('select * from plan')
	rows=cur.fetchall();
	return render_template('reviewtablecss.html',rows=rows)
...
def deleteshow():
	x='Jimjacob@gmail.com'
	try:
		with sql.connect('minidatabase.db')as con:
			cur=con.cursor()
			cur.execute('DELETE from reviews where email= ?',(x,))
			con.commit()
		...
		con.close()
#RECHARGE
@app.route('/recharge/<net>') #homepage 
def recharge(net):
	return ('https://www.airtel.in/prepaid-recharge/')
	# if net=='AIRTEL':
		# return render_template('https://www.airtel.in/prepaid-recharge/')
